...
class SharedObject extends EventDispatcher
{
	...
	**/
    ...
    #if sys
	/**
	 * What kind of path is the inputted path
     * 
     * ApplicationStorage - the generated application appdata storage
     * System - anywhere in the system
     * Local - located in the local filesystem
	 */
    public var pathType:PathType = ApplicationStorage;
    #end
	...
	**/
	public function clear():Void
	{
		...
		try
		{
			...
			#else
			var path = __getPath(__localPath, __name, pathType);
	...
	**/
	...
	@:noCompletion private static function __getPath(localPath:String, name:String, ?pathType:PathType = ApplicationStorage):String
	{
		#if lime
        #if sys
        if (pathType == SystemPath)
            return '$localPath/$name.sol';
        if (pathType == Local)
            return '${Sys.getCwd()}$localPath/$name.sol';
        #end
